...
class TechnicalIndicators:
    ...
    def _calculate_momentum_indicators(self, df):
        ...
        df['rsi_6'] = df['rsi_6'].fillna(df['rsi_6'].mean())
        df['rsi_14'] = df['rsi_14'].fillna(df['rsi_14'].mean())
        df['rsi_21'] = df['rsi_21'].fillna(df['rsi_21'].mean())
    ...
    def _calculate_volume_indicators(self, df):
        ...
        # CMF
        df['cmf'] = \
            ta.volume.ChaikinMoneyFlowIndicator(high=df['high'], low=df['low'], close=df['close'],
                                                volume=df['volume']).chaikin_money_flow()

        # MFI
        df['mfi'] = \
            ta.volume.MFIIndicator(high=df['high'], low=df['low'], close=df['close'],
...
class HybridAIModels:
    ...
    def predict(self, features):
        ...
        if isinstance(features, pd.DataFrame):
            # For a Pandas DataFrame, fill NaNs with the mean of each column
            print("features is a dataframe")
            features = features.fillna(features.mean(numeric_only=True))
            # Handle any remaining NaNs if the whole column was NaN (unlikely but safe)
            for col in features.select_dtypes(include=['object']):
                features[col] = features[col].fillna("missing")
            print("Remaining NaNs:", features.isna().sum().sum())
            print(f"number of infinite values : {np.isinf(features.to_numpy()).sum()}")
            # Replace infinities with NaN first
            features = features.replace([np.inf, -np.inf], np.nan)

            # Fill NaNs with column means (numeric only)
            features = features.fillna(features.mean(numeric_only=True))

            # Backstop: fill any remaining NaNs with 0
            features = features.fillna(0)
            print("Remaining NaNs:", features.isna().sum().sum())
            print("Remaining infinities:", np.isinf(features.to_numpy()).sum())

        '''elif isinstance(features, np.ndarray):
            # For a NumPy array, you need to calculate the mean of each column first
            print("features is a numpy array")
            features[np.isinf(features)] = np.nan
            col_means = np.nanmean(features, axis=0)
            # Find locations that are NaN
            nan_indices = np.where(np.isnan(features))
            # Replace the NaN values with the mean of their respective columns
            features[nan_indices] = col_means[nan_indices[1]]
            # Fill any remaining NaNs with 0 if necessary
            features[np.isnan(features)] = 0
            print("after numpy array")'''

        # 2. Replace any infinite values (inf or -inf) with a standard number
        #    The scaler simply cannot handle 'infinity'
        # Note: If your features contain legitimate extreme outliers, replacing with the mean might be better than 0,
        # but replacing inf with 0 usually works for trading data.

        # 3. Final check before scaling
        '''if not np.isfinite(features).all():
            print("CRITICAL DEBUG: Data still contains non-finite values after cleaning!")
            # Investigate which exact features are causing issues if this prints
        else:
            print("DEBUG: Data successfully cleaned, proceeding to scaling.")'''
        ...
        try:
            ...
            if features.shape[0] >= 30:
                sequence = features[-30:].to_numpy().reshape(1, 30, -1)
                tslm_pred = self.models['tslm'].predict(sequence, verbose=0)
                print(f"tslm_pred:{tslm_pred}")
                predictions['tslm'] = tslm_pred.item()
            print("tslm Predictions")
            # Weighted ensemble
...
def _display_results(data, indicators, signal, ai_prediction, confidence):
        ...
        print(f"{'-' * 80}")
        '''rsi_series = indicators.get('rsi_14', 0)
        latest_rsi_value = rsi_series.iloc[-1].item()
        macd_value = indicators.get('macd', pd.Series([0])).iloc[-1].item()
        bb_pos_value = indicators.get('bb_position', pd.Series([0])).iloc[-1].item()
        vwap_dist_value = indicators.get('vwap_distance', pd.Series([0])).iloc[-1].item()
        volume_ratio_value = indicators.get('volume_ratio', pd.Series([0])).iloc[-1].item()'''
        ...
        try:
            while is_running:
                if cycle_count == 0:
                    ...
                    #df_final =  df_final_history.tail(10000)
                    ...
                    TechnicalIndicatorsValue_100 = TechnicalIndicatorsValue[-100:]
                    ...
                    if ti_data:
                        ...
                        print("before the prediction call")
                        ...
                        # 6. Display results
                    _display_results(df_final, ti_data, signal, ai_prediction, confidence)
