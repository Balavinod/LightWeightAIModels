...
class TechnicalIndicators:
    ...
    def calculate_all_indicators(self, df):
        ...
        try:
            ...
            # 1. TREND INDICATORS
                ...
                print("Calculating trend indicators is completed")
            except Exception as e:
                print(f"Calculating trend indicators error: {e}")
            try:
            # 2. MOMENTUM INDICATORS
                df = self._calculate_momentum_indicators(df)
                print("Calculating momentum indicators is completed")
            except Exception as e:
                print(f"Calculating momentum indicators error: {e}")

            try:
            # 3. VOLATILITY INDICATORS
                df = self._calculate_volatility_indicators(df)
                print("Calculating volatility indicators is completed")
            except Exception as e:
                print(f"Calculating volatility indicators error: {e}")

            # 4. VOLUME INDICATORS
            try:
                df = self._calculate_volume_indicators(df)
                print("Calculating volume indicators is completed")
            except Exception as e:
                print(f"Calculating volume indicators error: {e}")
    ...
    def _calculate_trend_indicators(self, df):
        ...
        df['vwap_distance'] = df['vwap_distance'].replace([np.inf, -np.inf], np.nan).fillna(0)
        return df
    ...
    def _calculate_volatility_indicators(self, df):
        ...
        df['bb_position'] = df['bb_position'].replace([np.inf, -np.inf], np.nan).fillna(0)
        df['bb_width'] = df['bb_upper'] - df['bb_lower']

        # ATR (Average True Range)
        df['atr'] = \
        ta.volatility.AverageTrueRange(high=df['high'], low=df['low'], close=df['close']).average_true_range()
    ...
    def _calculate_volume_indicators(self, df):
        ...
        # OBV
        df['obv'] = \
        ta.volume.OnBalanceVolumeIndicator(close=df['close'], volume=df['volume']).on_balance_volume()

        # CMF
        df['cmf'] = \
        ta.volume.ChaikinMoneyFlowIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume']).chaikin_money_flow()

        # MFI
        df['mfi'] = \
        ta.volume.MFIIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume']).money_flow_index()
        return df
...
class HybridAIModels:
    ...
    def _initialize_models(self, scaler_path='scaler.pkl', models=None):
            ...
            self.models['tslm'] = tf.keras.Sequential([
                ...
                tf.keras.layers.Input(shape=(30, 18)),
    ...
    def prepare_features(self, historical_df):
        ...
        features = []
        # Calculate price-based features vectorized over the whole history
        historical_df['price_change'] = historical_df['close'].pct_change()
        historical_df['price_range'] = historical_df['high'] - historical_df['low']
        # Example rolling mean for volume ratio:
        historical_df['volume_ratio_current'] = historical_df['volume'] / historical_df['volume'].rolling(
            window=20).mean()

        # Technical indicators as features
        feature_columns = ['rsi_14', 'macd', 'macd_histogram', 'bb_position', 'vwap_distance',
                           'stoch_k', 'williams_r', 'atr', 'volume_ratio', 'cmf', 'mfi',
                           'sma_5', 'sma_20', 'ema_8', 'ema_21', 'price_change', 'price_range', 'volume_ratio_current']

        self.feature_columns = feature_columns

        return historical_df[feature_columns].dropna()
    ...
    def train_models(self, features, targets):
        ...
        try:
            ...
            print(f"Unique target values before XGBoost fit: {unique_targets}")
            # Train models
...
async def main():
    ...
    load_dotenv("C:\\Users\\Administrator\\AITraining.env")
        ...
        try:
            while is_running:
                    ...
                    try:
                        ...
                        print(f"all technical indicators calculated:{len(TechnicalIndicatorsValue)}")
                    except Exception as e:
                        print("TechnicalIndicators creating issue in main loop:{e}")
                    if TechnicalIndicatorsValue is not None and not TechnicalIndicatorsValue.empty:
                        logger.logger.info("Indicator calculation successfully all the value")
                    ti_data = {}
                    if len(TechnicalIndicatorsValue) > 0:
                        latest_row = TechnicalIndicatorsValue.iloc[-1]
                        print(f"latest row:{latest_row}")
                        if not latest_row.empty:
                            for col_name in TechnicalIndicatorsValue.columns:
                                ti_data[col_name] = latest_row[col_name]
                                print(f"creating dictionary:{ti_data[col_name]}")
                                print("creating dictionary completed")
                        if TechnicalIndicatorsValue is not None and not TechnicalIndicatorsValue.empty:
                            print("next step is preparing features")
                            features = ai_models.prepare_features(TechnicalIndicatorsValue)
                        ...
                        print(f"features_cleaned shape: {features_cleaned.shape}")
                        ...
                        #Targets = Targets.astype(int)
                        #Targets_series = pd.Series(Targets, index=df_final.index, name='target_label')

                        '''if len(features_cleaned) != len(Targets):
                            logger.logger.warning(f"Features ({len(features_cleaned)}) and Targets ({len(Targets)}) lengths do not match. Slicing to minimum length.")
                            min_length = min(len(features_cleaned), len(Targets))
                            X_train = features_cleaned[:min_length]
                            y_train = Targets[:min_length]
                            print(f"Size of data passed to train_models: {len(X_train)} records")
                        else:'''
                        #Targets_aligned = pd.Series(Targets, index=df_final.index, name = 'target_label')
                        Targets_aligned = pd.Series(Targets, index=df_final.index, name='target_label').reset_index(
                            drop=True)
                        #Targets_final = Targets_aligned.loc[features_cleaned.index]
                        #print(f"Targets_final shape: {Targets_final.shape}")
                        print(f"Targets_aligned shape: {Targets_aligned.shape}")
                        min_length = min(len(features_cleaned), len(Targets_aligned))
                        X_data_aligned = features_cleaned.iloc[:min_length].values
                        y_labels_aligned = Targets_aligned.iloc[:min_length].values
                        #X_data_aligned = features_cleaned.values
                        #y_labels_aligned = Targets_final.values
                        assert X_data_aligned.shape[0] == y_labels_aligned.shape[0], \
                            f"Mismatch: {X_data_aligned.shape[0]} vs {y_labels_aligned.shape[0]}"

                        print(f"Final X_data_aligned shape: {X_data_aligned.shape}, Final y_labels_aligned shape: {y_labels_aligned.shape}")
                        ai_models.train_models(X_data_aligned, y_labels_aligned)
                    if cycle_count == 1:
                        sys.exit(0)
        except Exception as e:
