...
class HybridAIModels:
    ...
    def _initialize_models(self):
        """Initialize all AI models optimized for Intel Ultra"""
        # TSLM (Time Series LSTM)
        self.models['tslm'] = tf.keras.Sequential([
            tf.keras.layers.LSTM(64, return_sequences=True, input_shape=(30, 15)),
        ...
        logging.info("🤖 Hybrid AI Models Initialized (TSLM + XGBoost + Random Forest)")
    ...
    def predict(self, features):
        ...
        try:
            ...
            # XGBoost prediction
            xgb_pred = self.models['xgboost'].predict_proba(scaled_features)[0][1]
            predictions['xgboost'] = xgb_pred

            # Random Forest prediction
            rf_pred = self.models['random_forest'].predict_proba(scaled_features)[0][1]
            predictions['random_forest'] = rf_pred

            # TSLM prediction (if we have sequence data)
            if features.shape[0] >= 30:
                sequence = features[-30:].reshape(1, 30, -1)
                tslm_pred = self.models['tslm'].predict(sequence, verbose=0)[0][0]
                predictions['tslm'] = tslm_pred
